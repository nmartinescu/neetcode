class Solution:
    def insert(self, intervals: List[List[int]], newInterval: List[int]) -> List[List[int]]:
        ...
        mergedInterval = None
        if i < len(intervals) and intervals[i][0] <= newInterval[1] and newInterval[0] <= intervals[i][1]:
            mergedInterval = [min(intervals[i][0], newInterval[0]), max(intervals[i][1], newInterval[1])]
            i += 1
            while i < len(intervals) and intervals[i][0] <= newInterval[1] and newInterval[0] <= intervals[i][1]:
